...
# Replay all failed task instances
#
...
echo "Replaying failed task instances"
echo "${BONITA_SIDECAR_URI}"

RESPONSE=$(curl --fail-with-body -X POST \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    "${BONITA_SIDECAR_URI}/api/bpm/failed-tasks/replay?${QUERY_STRING}")
# shellcheck disable=SC2181
if ! [[ $? == 0 ]]; then
    echo "Replay failure: ${RESPONSE}"
    exit 1
fi
