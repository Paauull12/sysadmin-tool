...
# Delete either all archived process instances or the ones with the optional version ($1)
#
...
echo "Deleting archived process instances with version ${VERSION:-unspecified}"
echo "${BONITA_SIDECAR_URI}"

RESPONSE=$(curl --fail-with-body -X DELETE \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    "${BONITA_SIDECAR_URI}/api/bpm/process-definitions/archived-instances?${QUERY_STRING}")
