...
# Delete either all process definitions or the ones with the optional version ($1)
#
...
echo "Deleting process definitions with version ${VERSION:-unspecified}"
echo "${BONITA_SIDECAR_URI}"

RESPONSE=$(curl --fail-with-body -X DELETE \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    "${BONITA_SIDECAR_URI}/api/bpm/process-definitions?${QUERY_STRING}")
